...
var LawRegEdit = avalon.define("LawRegEdit", function (vm) {
...
function initPannel(data) {
    ...
    $(".btn-confirm").click(function () {
        API.cutPic(data.fileUrl, cropData, function (data) {
            if (data.code == 0) {
                LawRegEdit.obj.newsImage = data.url;
                dialog.hide();
            }
        });
    });
...
module.exports = {
    ...
    render: function (param) {
        ...
        editor.ready(function () {
            LawRegEdit.obj = {type: 2, newsImage: "", content: "", title: ""};
            param && param.rid && queryInfoById(param.rid, function (result) {
                if (result.data) {
                    LawRegEdit.obj = result.data;
                    editor.setContent(LawRegEdit.obj.content || "");
                }
            });
        });
        uploadHandler();
