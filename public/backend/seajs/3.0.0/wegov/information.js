...
var Information = avalon.define("Information", function (vm) {
    ...
    vm.currentPage = 1;
    vm.edit = function (el) {
        avalon.router.navigate("/information_edit" + "/" + ((el && el.rid) || 0));
    }
    vm.del = function (rid) {
...
function queryNewsByType(param) {
    API.newsByType(param, function (result) {
        Information.list = result.data.list || [];
        Information.totalItems = Information.list.size() ? result.data.totalItems : 0;
    });
}
...
module.exports = {
    ...
    render: function (param) {
        Information.totalItems = 0;
        newsType = param.type || 1;
        Information.currentPage = param.currentPage || 1;
        queryNewsByType({type: newsType, currentPage: Information.currentPage, pageSize: pageSize});
    }
}
