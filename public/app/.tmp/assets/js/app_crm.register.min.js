...
var registerModel = avalon.define("register", function (vm) {
    vm.clientInfo={rid:0,clientNo:'',clientName:''};
    vm.settingConfig={mobile:"",codeInfo:"",codeRid:0,codeSendMessage:'发送验证码',codeSendEnable:false,codeSendClass:''};

    vm.init=function(){
        var type=getQueryString("type");
        if(type=="S"){
            appManager.login(function(client){
                vm.clientInfo.rid=client.rid;
                vm.clientInfo.clientName=client.clientName;
                vm.clientInfo.clientNo=client.clientNo;
            });
        }else{
            appManager.register(function(client){
                if(client) {
                    ...
                    vm.clientInfo.clientName=client.clientName;
                    vm.clientInfo.clientNo=client.clientNo;
                    alert('您好，'+vm.clientInfo.clientName+'欢迎进行注册');
                }
            });
        }
    }
    ...
    vm.confirmRegister=function(){
        var checkData={tel:vm.settingConfig.mobile,rid:vm.settingConfig.codeRid,friend:appManager.currentFriend,domain:appManager.currentDomain};
        checkData.clientId=appManager.currentUserInformation.rid;
        checkData.clientName=appManager.currentUserInformation.clientName;
        checkData.code=vm.settingConfig.codeInfo;
        $.ajax({
            type: "GET",
            url:'/weixin/checkCode',
            data:checkData,
            dataType:'json',
            success: function(data) {
                if(data.code==0){
                    var url=appManager.currentDomainInfo.attachedUrl;
                    if(!url){
                        url=appManager.convertUrlWithWeixinAuth('/app/app_crm/index.html');
